	...
	public static List<Project> extractProjects(InputStream in) {
		...
		try {
			...
			for (int row = 1; row < rows; row++) {
				...
				String supervisorFullName = sheet.getCell(2, row).getContents();
				
				maxAssignable = (maxAssignable == null
						|| maxAssignable.equals("") ? "1" : maxAssignable);
				Project project = new Project();
				project.setTitle(title);
				project.setMax(Integer.parseInt(maxAssignable));
				
				//look up supervisor key
				if(supervisorFullName != null && !supervisorFullName.trim().equals("")){
					Supervisor supervisor = supervisorDao.findByFullName(supervisorFullName.trim());
					Key<Supervisor> supervisorKey = null;
					if(supervisor != null){
