...
@Log4j
public class UploadSupervisorPanel extends Panel {
	
	public UploadSupervisorPanel(String id) {
		super(id);
		add(new UploadSupervisorForm("form"));
	}

	private class UploadSupervisorForm extends Form {
		...
		public UploadSupervisorForm(String id) {
			super(id);
			setMultiPart(true);
...
//					processFileUpload(upload, overwrite);
					UploadSupervisorForm.this.info("Upload Successfully");
				}
			});
			add(getDownloadLink());
		}
		...
		private DownloadLink getDownloadLink(){
			File file = new File(UploadSupervisorPanel.class.getResource("supervisor_upload_template.xls").getFile());
			String uuid = UUID.randomUUID().toString();
			String fileName = String.format("supervisor_upload_template%s.xls",uuid);
